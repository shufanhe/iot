...
async function getDevices(username) {
   ...
   for (let dev of devs) {
      ...
      let devdef = newdev.setup();
      console.log("NEW DEFINITION", devdef);
...
class SamsungDevice { 
   ...
   async analyzePresentation() {
      ...
      let devid = samsung_device.deviceid;
      ...
      if (conds != null) {
         for (let cond of conds) {
            ...
            if (cnm != null) condition_map[cnm] = cond;
   ...
   checkReference(obj) {
      ...
      if (sup != null && sup != "") {
         ...
         if (capid != null) reference_map[capid] = refcap;
   ...
   async analyzeCapabilities() {
      for (let comp of this.samsung_device.components) {
         if (comp.id != 'main') continue;
         for (let capid of comp.capabilities) {
            let cap = await loadCapability(this.user_id, capid);
            if (cap != null) {
               cap.componentid = comp.id;
   ...
   processCommandParameter(arg, cmd) {
      ...
      let cap = capability_map[capid];
      let ocap = condition_map[capid];
      if (cap.status != 'live') return null;
      let sub1 = cap.supportedValues;
      if (sub1 == null) sub1 = reference_map[capid];
      let pref = null;
      if (sub1 != null) {
         pref = {
            DEVICE: this.cat_dev.UID,
