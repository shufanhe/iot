...
async function displayHome(req, res, home, sid) {
  ...
  if (sid == null) {
    ...
    rows = await db.query(
      "SELECT * FROM iQsignSigns WHERE userid = $1 AND id = $2",
...
async function renderSignPage(req, res, signdata) {
  let rows0 = await db.query(
    "SELECT id, name FROM iQsignDefines " +
      "WHERE userid = $1 OR userid IS NULL",
    [req.session.user.id]
  );
  let rows1 = await db.query(
    "SELECT id,name FROM iQsignImages " + "WHERE userid = $1 OR userid IS NULL",
    [req.session.user.id]
  );
  let data = {
    sign: signdata,
...
async function doHandleUpdate(req, res) {
  ...
  let row = await db.query1(
    "SELECT * FROM iQsignSigns WHERE id = $1 " +
...
async function changeSign(signdata, cnts) {
  ...
  ss = ss.replace(/\t/g, " ");
  await db.query(
    "UPDATE iQsignSigns SET lastsign = $1, displayname = NULL " +
      " WHERE id = $2",
    [ss, signdata.id]
  );
...
async function handleSaveSignImage(req, res) {
  ...
  let signdata = rows[0];
  if (signdata.userid != req.body.signuser)
    return handleError(req, res, "Invalid user");
  if (signdata.namekey != req.body.signnamekey)
    return handleError(req, res, "Invalid namekey");

  let rows1 = await db.query(
    "SELECT * FROM iQsignDefines " + "WHERE userid = $1 AND name = $2",
    [uid, req.body.name]
  );
  if (rows1.length == 0) {
    let rows2 = await db.query(
      "INSERT INTO iQsignDefines(userid,name,contents) " +
        "VALUES ( $1, $2, $3 )",
      [uid, req.body.name, signdata.lastsign]
    );
    rows1 = await db.query1(
      "SELECT * FROM iQsignDefines " + "WHERE userid = $1 AND name = $2",
      [uid, req.body.name]
    );
    await db.query(
      "INSERT INTO iQsignUseCounts (defineid,userid) VALUES ($1,$2)",
      [rows1.id, uid]
    );
  } else {
    let rows3 = await db.query(
      "UPDATE iQsignDefines SET contents = $1 " +
...
async function handleLoadSignImage(req, res) {
  ...
  if (name == "*Current*") {
    ...
    if (nameid != null) {
      ...
      rows = await db.query(
        "SELECT * FROM iqSignDefines WHERE name = $1 AND userid = $2",
        [name, req.user.id]
      );
      if (rows.length == 0) {
        rows = await db.query(
          "SELECT * FROM iQsignDefines WHERE name = $1 AND userid IS NULL",
          [name]
        );
      }
    }
...
function getImageUrl(namekey) {
  return (
    "http://" + config.getWebHost() + "/iqsign/signs/image" + namekey + ".png"
  );
}

function getWebUrl(namekey) {
  return (
    "http://" + config.getWebHost() + "/iqsign/signs/sign" + namekey + ".png"
  );
}
...
async function getDisplayName(row) {
  ...
  if (sname == null) {
    let row0 = await db.query01(
      "SELECT * FROM iqSignDefines " + "WHERE contents = $1 AND userid = $2",
      [row.lastsign, row.userid]
    );
    if (row0 == null) {
      let row0 = await db.query01(
        "SELECT * FROM iqSignDefines " +
...
function displayCodePage(req, res) {
  ...
  if (req.session.user.id != req.body.signuser)
    return handleError(req, res, "Invalid user");

  let data = {
    user: req.body.signuser,
    sign: req.body.signid,
    key: req.body.signkey,
  };
...
async function createLoginCode(req, res) {
  ...
  if (req.body.signuser != req.session.user.id)
    handleError(req, res, "Invalid user");
  ...
  let code = config.randomString(
    24,
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
  );
  await db.query(
    "INSERT INTO iQsignLoginCodes ( code,userid,signid ) " +
      "VALUES ( $1, $2, $3 )",
    [code, uid, sid]
  );
